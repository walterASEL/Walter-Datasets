...
####################################
# Open netCDF 
# setting verbose=true provides details about the data in the netcdf file including  the varid. You need to 
# know the varid to select the variable you want to extract/summarize.  Note: the  dimensions x, y, time 
# also get a varid so you will need to subtract 3 from the varid of interest to get  the correct one.
#####################################
dat <- nc_open("Monthly_AvgMinTemp_1995-99_MPI.nc", write=TRUE,  readunlim=TRUE, verbose=TRUE)  

####################################
# Read data
# this loads all the data from the downloaded variable into the tmin object
#####################################
tmin <- dat$var[[1]]
tmin
...
head(df1, n = 10L)           # head(x, n = 6L, ...); head returns the first data  entries, x is the object, n sets the 
